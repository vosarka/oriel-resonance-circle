...
export default function Codex() {
  ...
  return (
    ...
    <div className="min-h-screen bg-black text-zinc-100">
      {/* Header */}
      <div className="glass-panel sticky top-0 z-10 border-t-2 border-t-primary/20">
        <div className="container py-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href="/carrierlock" className="px-4 py-2 bg-primary/20 border border-primary/30 rounded-lg hover:border-primary/50 hover:bg-primary/30 transition-all text-primary font-medium">
              Get Your Reading
            </Link>
          </div>
          ...
          <div className="relative">
            ...
            <Input
              ...
              className="pl-10 bg-zinc-900/50 border-primary/20 focus:border-primary/50 text-zinc-100 placeholder:text-zinc-500"
            />
          </div>
        </div>
      </div>
      ...
      <div className="container py-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {filteredCodons?.map((codon) => (
              <Link key={codon.id} href={`/codex/${codon.id}`}>
                <Card className="bg-zinc-900/50 border-primary/20 hover:border-primary/50 hover:bg-zinc-900/70 transition-all cursor-pointer group h-full">
                  <CardHeader>
                    <div className="flex items-start justify-between">
                      ...
                      <div className="text-xs text-zinc-500 font-mono bg-zinc-800/50 px-2 py-1 rounded border border-primary/10">
                        {codon.id}
                      </div>
                    </div>
                  </CardHeader>
                  <CardContent>
                    <p className="text-sm text-zinc-400 line-clamp-2 mb-3">{codon.essence}</p>
        ...
        {!isLoading && filteredCodons?.length === 0 && (
          <div className="text-center py-20">
            <p className="text-zinc-400 text-lg mb-2">No codons found</p>
